...
public class DividendoDivisor extends JFrame {
	...
	public DividendoDivisor() {
		...
		JLabel lblTitulo = new JLabel("Dividendo-Divisor");
		lblTitulo.setFont(new Font("Tahoma", Font.BOLD, 14));
		lblTitulo.setBounds(157, 11, 120, 14);
		...
		btnCalcular.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (primeiroDivisor == 0) throw new ArithmeticException();
				...
				try {
					...
					if (ultimoDivisor == 0) throw new ArithmeticException();
				} catch (NumberFormatException | ArithmeticException err) {
				...
				if (primeiroDividendo < ultimoDividendo && primeiroDivisor < ultimoDivisor) {
					for (int i = primeiroDividendo, j = primeiroDivisor; i <= ultimoDividendo && j <= ultimoDivisor; i += 2, j++) {
						if (j == 0) continue;
						
						resultado += i / Double.valueOf(j);
					}
